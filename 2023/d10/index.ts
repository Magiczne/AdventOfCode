...
import { rootDirectory, runExamples, runSolution } from '@magiczne/advent-of-code-ts-core/aoc'
...
const readData = (file: string, replaceAnimalWith: MapElement) => {
  const rawMap = readFileSync(file, 'utf-8').trim().split('\n')
...
const part1 = (file: string) => {
  ...
  const replaceAnimalWith = {
    '0.txt': MapElement.SEPipe,
    '1.txt': MapElement.SEPipe,
    'input.txt': MapElement.VPipe,
  }[fileName]
...
const part2 = (file: string) => {
  ...
  writeFileSync(join(rootDirectory, `2023/d10/output/${fileName}`), mapString, 'utf-8')
...
await runExamples(2023, '10', reader, part1, part2)
await runSolution(2023, '10', reader, part1, part2)
