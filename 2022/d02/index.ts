...
const calculateScore = (file: string): number => {
  return readData(file)
    .map((item: string): ParsedMove => {
      const parseItem = item.replace('X', 'A').replace('Y', 'B').replace('Z', 'C').split(' ')
...
await runExamples(2022, '02', reader, calculateScore, calculateScore2)
await runSolution(2022, '02', reader, calculateScore, calculateScore2)
