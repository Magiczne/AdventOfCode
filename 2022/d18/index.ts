...
const readPoints = (file: string): Array<Point> => {
  return readFileSync(file, 'utf-8')
    ...
    .split('\n')
    .map(line => {
      const point = line.split(',').map(coordinate => parseInt(coordinate, 10))
